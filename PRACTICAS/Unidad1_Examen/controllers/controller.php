...
class MvcController{
	...
	public function registroMaestroController(){
		...
		if(isset($_POST["nombreIngreso"])){
			...
			$datosController = array( "nombre"=>$_POST["nombreIngreso"],
											"apellidos"=>$_POST["apellidosIngreso"],
											"email"=>$_POST["emailIngreso"],
											"telefono"=>$_POST["telefonoIngreso"],
											"clave"=>$_POST["claveIngreso"]);
			//PASA POR PARAMETRO LOS DATOS Y LA TABLA QUE SE DESEA HACER EL REGISTRO DE LOS DATOS
			$respuesta = Datos::registroMaestroModel($datosController, "contacto");
			//VERIFICA QUE SEA CORRECTO
			if($respuesta == "success"){
				//MANDA UN OK EN ACTION A ENLACES
				error_reporting(0);
	...
	public function vistaAlumnosController(){
		...
		$respuesta = Datos::vistaAlumnosModel("proveedores");
		...
		echo'<tr>
				...
				<td>'.$item["email"].'</td>
				<td align="center" ><a href="index.php?action=editar&id='.$item["clave"].'"><button type="button" class="btn btn-info">Editar</button></a></td>
				<td align="center" ><a href="index.php?action=usuarios&idBorrar='.$item["clave"].'"><button type="button" class="btn btn-danger">Borrar</button></a></td>
				<td align="center" ><a href="index.php?action=verAlumnosMaterias&id='.$item["clave"].'"><button type="button" class="btn btn-info">Ver contactos</button></a></td>
			</tr>';
	...
	public function vistaMaestrosController(){
		//TRAE TODOS LOS ALUMNOS
		$respuesta = Datos::vistaMaestroModel("contacto");
		...
		echo'<tr>
				...
				<td>'.$item["telefono"].'</td>
				<td>'.$item["clave"].'</td>
				<td align="center" ><a href="index.php?action=editarMaestros&id='.$item["nombre"].'"><button type="button" class="btn btn-info">Editar</button></a></td>
				<td align="center" ><a href="index.php?action=maestro&idBorrar='.$item["nombre"].'"><button type="button" class="btn btn-danger">Borrar</button></a></td>
			</tr>';
...
public function borrarTutoriaController(){
	...
	public function editarAlumnosController(){
		...
		$respuesta = Datos::editarAlumnoModel($datosController, "proveedores");
		...
		echo'
				<div class="row-spacing col-lg-12">
					<div class="form-group col-lg-7">
					  ...
					  <input type="hidden" value="'.$respuesta["clave"].'" class="form-control" name="idEditar">
					</div>

					<div class="form-group col-lg-7">
						<label>Clave</label>
					  <input type="text" readyonly value="'.$respuesta["clave"].'" class="form-control" name="claveEditar">
					</div>
					...
					<div class="form-group col-lg-7">
						<label>RFC</label>
					  <input type="text" value="'.$respuesta["rfc"].'" class="form-control" name="rfcEditar" required>
					</div>

					<div class="form-group col-lg-7">
						<label>DIRECCION</label>
						<input type="text" value="'.$respuesta["direccion"].'" class="form-control" name="direccionEditar" required>
					</div>

					<div class="form-group col-lg-7">
						<label>EMAIL</label>
						<input type="text" value="'.$respuesta["email"].'" class="form-control" name="emailEditar" required>
					</div>

				</div>
			 <input style="margin-left:30px;" type="submit" type="button" class="btn btn-success" value="Actualizar" requiredx>';
	...
	public function editarMaestrosController(){
		...
		$datosController = $_GET["id"];
		//MANDA COMO PARAMETRO ESOS DATOS Y LLAMA AL METODO EDITARUSUARIOMODEL
		$respuesta = Datos::editarMaestrosModel($datosController, "contacto");
		//GENERA LOS INPUT PARA EDITAR LOS VALORES DEL REGISTRO
		...
		echo'
				<div class="row-spacing col-lg-12">
					<div class="form-group col-lg-7">
					  ...
					  <input type="hidden" value="'.$respuesta["nombre"].'" class="form-control" name="idEditar">
					</div>

					<div class="form-group col-lg-7">
						<label>NOMBRE DEL CONTACTO</label>
					  <input type="text" readonly	 value="'.$respuesta["nombre"].'" class="form-control" name="nombreEditar">
					</div>

					<div class="form-group col-lg-7">
						<label>APELLIDOS</label>
					  <input type="text" value="'.$respuesta["apellidos"].'" class="form-control" name="apellidosEditar">
					</div>
					...
					<div class="form-group col-lg-7">
						...
						<input type="text" value="'.$respuesta["telefono"].'" class="form-control" name="telefonoEditar">
					</div>

					<div class="form-group col-lg-7">
						<label>CLAVE</label>
						<input type="text" value="'.$respuesta["clave"].'" class="form-control" name="claveEditar">
					</div>
	...
	public function actualizarAlumnoController(){
		...
		if(isset($_POST["nombreEditar"])){
			...
			$datosController = array( "clave"=>$_POST["claveEditar"],
							          "nombre"=>$_POST["nombreEditar"],
				                      "rfc"=>$_POST["rfcEditar"],
															 "direccion"=>$_POST["direccionEditar"],
				                      "email"=>$_POST["emailEditar"]);
			//LLAMA AL METODO DENTRO DEL MODELO DE ACTULAIZAR USUARIO
			$respuesta = Datos::actualizarAlumnoModel($datosController, "proveedores");
...
//					echo  "<script> windows.location = 'index.php?action=usuarios'</script>";
	...
	public function actualizarMaestroController(){
		...
		if(isset($_POST["nombreEditar"])){
			//GUARDA EN UN ARRAY LOS VALORES PARA ACTUALIZAR LOS DATOS
			$datosController = array( "nombre"=>$_POST["nombreEditar"],
							          			"apellidos"=>$_POST["apellidosEditar"],
				                      "email"=>$_POST["emailEditar"],
				                      "telefono"=>$_POST["telefonoEditar"],
															"clave"=>$_POST['claveEditar']);
			//LLAMA AL METODO DENTRO DEL MODELO DE ACTULAIZAR USUARIO
			$respuesta = Datos::actualizarMaestroModel($datosController, "contacto");
			//SI ES CORRECTO MANDA ACTION CAMBIO A ENLACES
			if($respuesta == "success"){
...
//					echo  "<script> windows.location = 'index.php?action=usuarios'</script>";
	...
	public function borrarAlumnoController(){
		...
		if(isset($_GET["idBorrar"])){
			...
			$respuesta = Datos::borrarAlumnoModel($datosController, "proveedores");
	...
	public function borrarMaestroController(){
		...
		if(isset($_GET["idBorrar"])){
			...
			$datosController = $_GET["idBorrar"];
			//LLAMA AL METODO PARA BORRAR UN USUARIO
			$respuesta = Datos::borrarMaestroModel($datosController, "contacto");
			//EN CASO DE SER CORRECTO REGRESA A LA VISTA DE USUARIOS
			if($respuesta == "success"){
				error_reporting(0);
